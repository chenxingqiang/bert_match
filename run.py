...
flags.DEFINE_integer("topK", 10000, "Total batch size for training.")
...
flags.DEFINE_integer("epoch", 40, "Maximum number of eval steps.")
...
tf.flags.DEFINE_string(
    ...
    "url.")
tf.flags.DEFINE_string(
    "dataset_name","all", "train dataset name for separate the save pb model ")
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        # Compute predictions.
            ...
            topK_item_ids = tf.nn.top_k(masked_lm_log_probs, k=FLAGS.topK).indices
            predictions = {
                'TopK_predictions': topK_item_ids}
            export_outputs = {
                'prediction': tf.estimator.export.PredictOutput(predictions)
            }
            output_spec = tf.estimator.EstimatorSpec(mode, predictions=predictions, export_outputs=export_outputs)

        else:
            raise ValueError("Only TRAIN and EVAL PREDICT modes are supported: %s" %
                             (mode))
...
def input_dataset_file(FLAGS):
    """
    :param FLAGS:
    :return:
    """
    if isinstance(FLAGS.train_input_file, str) and os.path.isdir(FLAGS.train_input_file):
        train_files = []
        for x in os.listdir(FLAGS.train_input_file):

            if FLAGS.signature[:-3]+".train.tfrecord" in x:
                train_files += [os.path.join(FLAGS.train_input_file,x)]
                print(train_files)
    else:
        train_files = FLAGS.train_input_file
        print(train_files)

    if isinstance(FLAGS.test_input_file, str) and os.path.isdir(FLAGS.test_input_file):
        eval_files = []
        for x in os.listdir(FLAGS.test_input_file):
            if FLAGS.signature[:-3]+".test.tfrecord" in x:
                eval_files += [os.path.join(FLAGS.test_input_file, x)]

    else:
        eval_files = FLAGS.test_input_file
...
def input_fn_builder(input_files,
    ...
    def input_fn(params):
        ...
        if is_training:
            ...
            d = d.shuffle(buffer_size=100).repeat(epoch)
...
def main(_):
    ...
    train_input_files,test_input_files = input_dataset_file(FLAGS)

    tf.logging.info("*** train Input Files ***")
    for input_file in list(train_input_files):
        tf.logging.info("  %s" % input_file)

    tf.logging.info("*** test Input Files ***")
    for input_file in list(test_input_files):
        tf.logging.info("  %s" % input_file)
    ...
    if FLAGS.do_eval:
        ...
        eval_input_fn = input_fn_builder(
            input_files=test_input_files[0:2],
            max_seq_length=FLAGS.max_seq_length,
            max_predictions_per_seq=FLAGS.max_predictions_per_seq,
            is_training=False)
        ...
        def serving_input_fn():
            inputs = {
                "info":
                    tf.placeholder(tf.int32, shape=[None, 1], name="info"),
                "input_ids":
                    tf.placeholder(tf.int32, shape=[None, FLAGS.max_seq_length], name="input_ids"),
                "input_mask":
                    tf.placeholder(tf.int32, shape=[None, FLAGS.max_seq_length], name="input_mask"),
                "masked_lm_positions":
                    tf.placeholder(tf.int32, shape=[None, FLAGS.max_predictions_per_seq], name="input_mask"),
                "masked_lm_ids":
                    tf.placeholder(tf.int32, shape=[None, FLAGS.max_predictions_per_seq], name="masked_lm_ids"),
                "masked_lm_weights":
                    tf.placeholder(tf.float32, shape=[None, FLAGS.max_predictions_per_seq], name="masked_lm_weights"),
            }
            return tf.estimator.export.ServingInputReceiver(inputs, inputs)

        estimator_base_path = os.path.join(FLAGS.checkpointDir, 'pb_model_output', FLAGS.dataset_name)
