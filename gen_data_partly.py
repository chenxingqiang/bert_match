...
flags.DEFINE_string(
    ...
    "dataset name.")
flags.DEFINE_string(
    "all_vocab_file", 'ml-1m-all.vocab',
    "all vocab dataset name.")

flags.DEFINE_boolean(
    "all_vocab", True,
    "if or not use all vocab dataset name.")
...
def main():
    ...
    if FLAGS.all_vocab_file is not None:
        with open(FLAGS.all_vocab_file, 'rb') as input_file:
            all_vocab = pickle.load(input_file)
    part_vocab = FreqVocab(user_test_data)
    ...
    print('part vocab size:{}, part user size:{}, part item size:{}, part item with other size:{}'.
          format(part_vocab.get_vocab_size(),
                 part_vocab.get_user_count(),
                 part_vocab.get_item_count(),
                 part_vocab.get_item_count() + part_vocab.get_special_token_count()))
    vocab_file_name = output_dir + dataset_name + version_id + '.vocab'
    print('part vocab pickle file: ' + vocab_file_name)
    with open(vocab_file_name, 'wb') as output_file:
        pickle.dump(part_vocab, output_file, protocol=2)

    his_file_name = output_dir + dataset_name + version_id + '.his'
    print('part test data pickle file: ' + his_file_name)
